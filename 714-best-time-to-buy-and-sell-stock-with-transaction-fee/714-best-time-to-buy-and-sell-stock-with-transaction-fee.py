    ...
    def maxProfit(self, prices: List[int], fee: int) -> int:
        ...
        def get_max_profit(day: int, stock_on_hand: int) -> int:
            ...
            if (day, stock_on_hand) not in stock_cache:
                do_nothing = get_max_profit(day + 1, stock_on_hand)

                if stock_on_hand:
                    involve_in_transaction = get_max_profit(day + 1, stock_on_hand=0) + prices[day] - fee
                else:
                    involve_in_transaction = get_max_profit(day + 1, stock_on_hand=1) - prices[day]
                
                stock_cache[(day, stock_on_hand)] = max(do_nothing, involve_in_transaction)
